...
class SpringODECell(ODECell):
    ...
    def forward(self, poss, vels):
        poss = list(th.split(poss, poss.shape[1] // 2, dim=1))
        vels = list(th.split(vels, vels.shape[1] // 2, dim=1))
        for _ in range(5):
            norm = th.sqrt(th.abs(th.sum(th.square(poss[0] - poss[1]), dim=-1, keepdim=True)))
            direction = (poss[0] - poss[1]) / (norm + 1e-4)
            force = th.exp(self.k) * (norm - 2 * th.exp(self.equil)) * direction
            vels[0] = vels[0] - self.dt / 5 * force
            vels[1] = vels[1] + self.dt / 5 * force

            poss[0] = poss[0] + self.dt / 5 * vels[0]
            poss[1] = poss[1] + self.dt / 5 * vels[1]

        poss = th.concat(poss, dim=1)
        vels = th.concat(vels, dim=1)
        return poss, vels
